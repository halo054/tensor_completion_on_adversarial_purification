...
#            raise NotImplementedError('unknown diffusion type')
    ...
    def forward(self, x):
        ...
        x_re = []
        length = np.shape(x_np)
        for image_index in range(length[0]):
            print(image_index)
            image_temp = x_np[image_index]
...
#            image_temp = image_temp.requires_grad_(True)
        ...
        x_re = F.interpolate(x_re, size=(32, 32), mode='bilinear', align_corners=False)
...
def eval_autoattack(args, config, model, x_val, y_val, adv_batch_size, log_dir):
    ...
    x_adv_resnet = adversary_resnet.run_standard_evaluation(x_val[0:250,:,:,:], y_val[0:250], bs=adv_batch_size)
...
def eval_pgd(args, config, model, x_val, y_val, adv_batch_size, log_dir):
    ...
    adversary_resnet = PGD(resnet, eps=8/255, alpha=2/225, steps=10, random_start=True)
    ...
    init_acc = get_accuracy(model, x_val, y_val, bs=adv_batch_size)
    print('initial accuracy: {:.2%}, time elapsed: {:.2f}s'.format(init_acc, time.time() - start_time))
    adversary_sde = PGD(model, eps=8/255, alpha=2/225, steps=10, random_start=True)
...
def robustness_eval(args, config):
    ...
    # load data
    x_val, y_val = load_data(args, adv_batch_size,0)
    '''
    ...
    elif args.attack_version == 'PGD':
        eval_pgd(args, config, model, x_val[0:64,:,:,:], y_val[0:64], adv_batch_size, log_dir)
    else:
